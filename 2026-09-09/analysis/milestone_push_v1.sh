#!/bin/bash
# v1 재개(120k→, 1만 스텝 마일스톤) 자동 커밋·푸시.
...
MDIR=$REPO/2026-09-09/run_v1_resume/milestones
NDIR=$REPO/2026-09-09/checkpoints
LOG=$REPO/2026-09-09/results/milestone_push.log
TRAIN_PID=${1:-0}
...
commit_push() {   # $1 = 커밋 메시지 제목
  cd "$REPO" || return 1
  cp -f "$REPO/2026-09-09/train_v1_resume.log" "$REPO/2026-09-09/train_v1_resume.log" 2>/dev/null
  git add -A 2026-09-08/ 2026-09-09/ >> "$LOG" 2>&1
  if git diff --cached --quiet; then say "변경 없음 — 건너뜀"; return 0; fi
  local STEP ACC
  STEP=$(cat 2026-09-09/train_v1_resume*.log 2>/dev/null | grep -o 'step [0-9]*' | tail -1 | grep -o '[0-9]*')
  ACC=$(cat 2026-09-09/train_v1_resume*.log 2>/dev/null | grep "^\[EVAL\]" | tail -1)
...
while true; do
  ...
  for pt in "$MDIR"/step_*.pt; do
    ...
    if python "$REPO/checkpoints/ckpt_npz.py" pack "$pt" "$npz" \
         --note "v1.7 (agree복원·post·상수게이지·agree제거·잔차흔적). train_v16.py" >> "$LOG" 2>&1; then
  ...
  NOW=$(date +%s)
  if [ "$NEW" = "1" ]; then
    commit_push "v1.7 마일스톤 체크포인트(npz)·외삽·로그"
    LAST_HEARTBEAT=$NOW
  elif [ $(( NOW - LAST_HEARTBEAT )) -ge 3600 ]; then
    commit_push "v1.7 학습 로그 (자동)"
    LAST_HEARTBEAT=$NOW
  fi
  ...
  if [ "$TRAIN_PID" != "0" ] && ! kill -0 "$TRAIN_PID" 2>/dev/null; then
    ...
    shopt -s nullglob
    for pt in "$MDIR"/step_*.pt; do
      base=$(basename "$pt" .pt); npz="$NDIR/v1r_${base}.npz"
      [ -f "$npz" ] || python "$REPO/checkpoints/ckpt_npz.py" pack "$pt" "$npz" --note "v1.6 최종" >> "$LOG" 2>&1
    done
    shopt -u nullglob
    commit_push "v1.7 학습 종료 — 최종 체크포인트·로그"
