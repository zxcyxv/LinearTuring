...
ROOT=os.path.dirname(os.path.dirname(os.path.abspath(__file__))); SP=os.environ.get("LT_OUT", os.path.join(ROOT,"runs","analysis")); os.makedirs(SP, exist_ok=True)
torch.set_grad_enabled(False)
NB,SEG,BPS=512,128,8; KTRAIN=16*BPS
s=importlib.util.spec_from_file_location("tk",os.path.join(ROOT,"lt/train.py"))
tk=importlib.util.module_from_spec(s); s.loader.exec_module(tk)
sys.path.insert(0,os.path.join(ROOT,"lt")); from ckpt_npz import load as npz_load
...
z=np.load(os.path.join(ROOT,"data/sudoku_lt_1k.npz"))
...
np.savez_compressed(os.path.join(SP,"phase_oscillation.npz"),ru16=ru16,rw16=rw16,rp16=rp16,ruF=ruF,rwF=rwF,rpF=rpF,P=P,X=X,Y=Y)
