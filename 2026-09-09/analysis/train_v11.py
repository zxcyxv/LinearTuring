"""[v1.1 = 2026-09-09] train_0901.py(v1) 에서 legacy_gauge 만 False 로. 나머지(pre 순서·agree·별도 β 창·거리감쇠·흔적 없음·1벌) 전부 v1.
질문: v1 의 제약 끌개(위반 단조 감소·외삽)가 √d 고정 게이지(단서 불변) 아래서도 유지되는가.
"""
# -*- coding: utf-8 -*-
...
CFG = dict(
    ...
    mlp_expansion=4.0,
    alpha_init=0.1,
    legacy_gauge=False,   # [v1.1] 학습 재주입 계수·학습 γ 제거 → √d 고정 게이지 (v1.7 과 같은 게이지, 동역학은 v1)
    inj_gate_init=0.25,
    gamma_init=0.1,
    ...
    stdp=True,
    stdp_target="faithful",           # [2026-09-05] Γ = a (agree 항 ⟨v̂_t,v̂_n⟩ 제거). STDP 는 부호가 창(타이밍)에서만 나와야 하고
    ...
    block_order="pre",          # [2026-09-04 추가 플래그] 블록 안 연산 순서. post = 주입→스텝→경계 (판독이 쌍선형 직후,
    ...
    # ---- 실행/저장
    out_dir="/workspace/LinearTuring/2026-09-09/run_v11",                 # None → /kaggle/working/checkpoints (없으면 ./lt_checkpoints)
    ...
    milestone_every=10000,        # [2026-09-05] 이 주기로 out_dir/milestones/step_N.pt 에 **영구** 저장 (keep_last 정리 대상 아님)
