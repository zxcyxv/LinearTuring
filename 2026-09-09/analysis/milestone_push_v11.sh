...
MDIR=$REPO/2026-09-09/run_v11/milestones
...
commit_push() {   # $1 = 커밋 메시지 제목
  cd "$REPO" || return 1
  cp -f "$REPO/2026-09-09/train_v11.log" "$REPO/2026-09-09/train_v11.log" 2>/dev/null
  git add -A 2026-09-08/ 2026-09-09/ >> "$LOG" 2>&1
  if git diff --cached --quiet; then say "변경 없음 — 건너뜀"; return 0; fi
  local STEP ACC
  STEP=$(cat 2026-09-09/train_v11*.log 2>/dev/null | grep -o 'step [0-9]*' | tail -1 | grep -o '[0-9]*')
  ACC=$(cat 2026-09-09/train_v11*.log 2>/dev/null | grep "^\[EVAL\]" | tail -1)
...
while true; do
  ...
  NOW=$(date +%s)
  if [ "$NEW" = "1" ]; then
    commit_push "v1.1 마일스톤 체크포인트(npz)·외삽·로그"
    LAST_HEARTBEAT=$NOW
  elif [ $(( NOW - LAST_HEARTBEAT )) -ge 3600 ]; then
    commit_push "v1.1 학습 로그 (자동)"
    LAST_HEARTBEAT=$NOW
  fi
  ...
  if [ "$TRAIN_PID" != "0" ] && ! kill -0 "$TRAIN_PID" 2>/dev/null; then
    ...
    shopt -s nullglob
    for pt in "$MDIR"/step_*.pt; do
      base=$(basename "$pt" .pt); npz="$NDIR/v11_${base}.npz"
