...
P=sorted(glob.glob(os.path.join(ROOT,"2026-09-09/run_v17/step_*.pt")), key=os.path.getmtime)[-1]
...
for si,acc,ex,ch in rows:
    if si in (1,2,4,8,16,24,32,48,64,80,96,112,128):
        print(f"{si:5d} {acc:8.4f} {ex:6d} {100*ex/NB:8.2f} {ch:8.5f}" + ("  <-train" if si==16 else ""))
e16=rows[15][2]; eb=max(r[2] for r in rows); sb=[r[0] for r in rows if r[2]==eb][0]
print(f"\nseg16 {e16}/512 ({100*e16/NB:.2f}%)   best {eb}/512 ({100*eb/NB:.2f}%) @seg{sb}   +{eb-e16}")
print(f"오차 감소율 = {100*((NB-e16)-(NB-eb))/(NB-e16):.1f}%   ({time.time()-t0:.0f}s)")
