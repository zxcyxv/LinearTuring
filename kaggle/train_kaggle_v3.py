# -*- coding: utf-8 -*-
"""LinearTuring v3 — Kaggle 단독 학습 스크립트 (2026-09-06). 플래그 없음.

모델 (LTConfig docstring 참조):
    읽기 커널 a = Re Z (ψ),  쓰기 커널 a⊥ = −Im Z (ψ+π/2, 성분별 90°)
    쓰기  w ← (1−δ)w + δ·g·a⊥/(|Z|+ε)      위상만 기억
    읽기  a_eff = w·a                       K·Γ
...
@dataclass
class LTConfig:
    """v3 (2026-09-06). 플래그 없음. 구조는 아래 한 가지:

        읽기 커널  a   = Σ_j r_tj r_nj cos(Δφ_j + θ_j·Δ + ψ_j)              = Re Z
        쓰기 커널  a⊥  = Σ_j r_tj r_nj cos(Δφ_j + θ_j·Δ + ψ_j + π/2)        = −Im Z   (성분별 90°, Gushchin G = −∫Γ)
        쓰기       w ← (1−δ) w + δ · g · a⊥ / (|Z| + ε)                     위상만 기억, 코히런스 |Z| 는 읽기에서
        읽기       a_eff = w · a                                             K·Γ
        전달       h ← h + Wᵀ Σ_n a_eff,tn W h_n ;  h ← boundary(h) ;  h ← Φ(h)

    g 는 대각(자기 관성)·비대각(쌍 결합) 따로. 게이지 상수: 주입 √d, Φ 반경 √d (γ = 1/d). 거리 감쇠 없음.
    근거: 2026-09-06/이론.md §8–9, README §3."""
    batch_size: int
    seq_len: int
    vocab_size: int
    ...
    znorm_eps: float = 0.08     # 쓰기 목표 |Z| 정규화 ε — ρ 전형값 근처. ρ≈0 쌍의 무작위 위상이 단위 크기로 쌓이는 것을 막는다
...
class LT_Inner(nn.Module):
    ...
    def attn_xy(self, xy, kc):
        """복소 스코어 Z_tn = Σ_j q̂_tj conj(k̂_nj) 의 (Re Z, −Im Z) — 각각 읽기 커널 a 와 쓰기 커널 a⊥ [B,H,T,T].
        a⊥ = a(ψ+π/2) 와 같다: Re[q e^{iπ/4} conj(k e^{−iπ/4})] = Re[iZ] = −Im Z."""
        ...
        ap = torch.einsum('bthj,bnhj->bhtn', qx, ky) - torch.einsum('bthj,bnhj->bhtn', qy, kx)   # −Im Z
        return a, ap
    ...
    def step(self, L, h, AB, kc, w=None, fresh=None):
        ...
        a, ap = self.attn(h, AB, kc)                                        # 읽기 커널 Γ = Re Z,  쓰기 커널 = −Im Z
        v = torch.einsum('btd,hcd->bthc', h, L.w_sh)
        # ---- 쓰기: 위상만. tgt = g · a⊥ / (|Z| + ε).  g 는 대각·비대각 따로
        rho = torch.sqrt(a.float().pow(2) + ap.float().pow(2) + 1e-12).to(a.dtype)   # +1e-12: a=a⊥=0 쌍에서 sqrt 역전파 0×inf=NaN 방지
        gain = F.softplus(L.gain_raw) * (1 - self.eye) + F.softplus(L.gain_diag_raw) * self.eye     # [H,T,T]
        tgt = gain.unsqueeze(0) * ap / (rho + self.config.znorm_eps)
...
NO_DECAY_KEYS = ("psi", "theta", "gain_raw", "gain_diag_raw", "eta_raw")
...
def extrapolate(base, eval_in, eval_lb, cfg, rank, world_size, device, step: int,
    ...
    lines = [f"# seg_extrap  step={step}  weights=ema  segs={segs}  n={N}  elapsed={time.time()-t0:.0f}s",
             ...
             f"layers={cfg['num_layers']} znorm_eps={cfg['znorm_eps']} eta_init={cfg['eta_init']} gain_init={cfg['gain_init']}",
    ...
    if resume_path is None and cfg["resume_from"] is None and cfg["scan_kaggle_input"]:
        ...
        if cand:
            try:
                ...
                c = {}
            keys = ("hidden_size", "num_heads", "loops", "blocks_per_seg", "grid", "num_layers", "mlp_expansion", "znorm_eps")
