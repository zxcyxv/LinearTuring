...
(S) 스텝
    ...
    a_tn   = D(Δ_tn) · Σ_j Re[ conj(û_t,j) û_n,j · e^{i(θ_j·Δ_tn + ψ_j)} ]      읽기 커널: 위상차 + 위치 회전 + 상수 위상 ψ
...
누적한다. 블록 축에서 부호가 흔들리는 결합은 상쇄되고 지속되는 것만 남는다. 세그먼트를 더 돌릴수록 이 누적이 더 길어진다 —
그것이 외삽의 기제라는 것이 이 모델의 가설이고, `docs/stdp_fourier.md` 는 이 쓰기 경로가 STDP 의 안쪽 적분을
주파수 영역에서 쓴 것과 항별로 대응함을 유도한다. `agree` 와 읽기의 `λ` 혼합은 그 대응 밖에 있다 (문서 끝에 적어 두었다).
