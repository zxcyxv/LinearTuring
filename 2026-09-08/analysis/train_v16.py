...
CFG = dict(
    ...
    # ---- 실행/저장
    out_dir="/workspace/LinearTuring/2026-09-08/run_v16",                 # None → /kaggle/working/checkpoints (없으면 ./lt_checkpoints)
...
class LT_Inner(nn.Module):
    ...
    def step(self, L, h, AB, kc, w=None, fresh=None, kcb=None, ztr=None, apply_phi=True):
        ...
        근거: STDP 는 전달을 지금 신호로, 가소성을 이력으로 한다.
          ...
          · agree 는 활동 인자 자리인데 이 계엔 활동변수가 없고(진폭 두 번 정규화), [−1,1] 이라
            창이 독점해야 할 부호를 침범한다. 제거.
        ...
        G   = self.attn_xy((zhx, zhy), kcb)                                 # 쓰기 창 a_β (β) — 흔적 주소
